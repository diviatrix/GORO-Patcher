...
- **Manifest signing (Ed25519)** — `plist.json` is now signed by the publisher (`hashfile genkey` / `sign`) and verified against a public key the admin supplies at runtime (`manifest_public_key` in `goro-config.json`, or `GORO_PATCHER_PUBKEY`) before any field is trusted. Release builds (`build.sh --release`) refuse unsigned or tampered manifests; dev builds skip verification so local, unsigned manifests keep working. Because the key is supplied per server at runtime rather than compiled in, each admin ships a release binary of their own (there is no generic pre-built release patcher).
